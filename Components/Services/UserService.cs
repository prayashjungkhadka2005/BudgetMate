    ...
    using BCrypt.Net;
        ...
        public class UserService
        {
            ...
            public User LoginUser(string username, string plaintextPassword)
            {
                ...
                if (user != null)
                {
                    bool isPasswordValid = BCrypt.Net.BCrypt.Verify(plaintextPassword, user.Password);
            ...
            public bool ClearDebt(int debtId)
            {
                try
                {
                    ...
                    if (debt != null)
                    {
                        // Get the available inflows
                        int availableInflows = GetTotalInflows();

                        // Ensure inflows are sufficient to clear the debt
                        if (debt.DebtAmount > availableInflows)
                        {
                            Debug.WriteLine("Insufficient inflows to clear the debt.");
                            return false;
                        }
                        ...
                        _database.Insert(new Transaction
                        {
                            ...
                            TransactionTitle = debt.DebtTransactionTitle,
                            TransactionDate = DateTime.Now.ToString("yyyy-MM-dd"),
                            Amount = -debt.DebtAmount, // Subtract the cleared debt from inflows
                            Type = "DebtCleared",
                            Tags = debt.SourceOfDebt,
                            Note = "Debt cleared successfully"
                        });

                        // Recalculate balance without starting a transaction
                        RecalculateBalance();
            ...
            //        // Total inflows are the sum of all credits minus cleared debts
            //        int totalInflows = _database.Table<Credit>()
            //            .Where(c => c.UserId == _currentUser.UserId)
            //            .Sum(c => c.CreditAmount);

            //        int totalClearedDebts = _database.Table<Debt>()
            //            .Where(d => d.UserId == _currentUser.UserId && d.isCleared)
            //            .Sum(d => d.DebtAmount);
            ...
            public int GetTotalInflows()
            {
                try
                {
                    ...
                    // Total inflows are the sum of all credits minus the total cleared debts
                    int totalCredits = _database.Table<Credit>()
                        .Where(c => c.UserId == _currentUser.UserId)
                        .Sum(c => c.CreditAmount);

                    int clearedDebts = _database.Table<Debt>()
                        .Where(d => d.UserId == _currentUser.UserId && d.isCleared)
                        .Sum(d => d.DebtAmount);

                    return totalCredits - clearedDebts;
                }
                catch (Exception ex)
                {
            ...
            //        int totalInflows = GetTotalInflows();
            //        int totalOutflows = GetTotalOutflow();
            //        int remainingDebts = GetRemainingDebt();

            //        int totalBalance = totalInflows - totalOutflows;
